...
function uploadCustomerImageFiles(fileList, csrfToken) {
    ...
    files.forEach(function (file) {
        ...
        $.ajax({
            ...
            success: function (response) {
                setCustomerUploadItemStatus(itemId, 'Fertig', 'text-green-700');
                if (response.image) {
...
function addCustomerUploadItem(id, name) {
    $('#imageUploadItems').prepend(`
        <div id="${id}" class="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 text-sm">
            <span class="truncate pr-3 text-slate-700">${customerEscapeHtml(name)}</span>
            <span class="upload-status text-slate-500">Lädt...</span>
        </div>
    `);
}

function setCustomerUploadItemStatus(id, text, className) {
